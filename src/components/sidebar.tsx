...
export default function Sidebar() {
  return (
    <nav className="flex flex-col bg-slate-800 md:border-r gap-3 border-slate-700 shadow-lg md:p-5 md:w-72 w-0 invisible md:visible">
  ...
  return (
    <Link
      ...
      className={`relative flex flex-col px-5 py-4 rounded-lg ${
        props.isActive ? "bg-slate-700" : "hover:bg-slate-400/5"
      }`}
    >
      ...
      <span
        className={`absolute inset-y-4 -left-px w-1 rounded-full bg-gradient-to-b ${
          props.isActive ? "bg-indigo-500" : "bg-slate-400/20"
        }`}
      ></span>
    </Link>
  );
